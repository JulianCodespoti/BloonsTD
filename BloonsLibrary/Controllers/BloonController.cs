    ...
    public class BloonController
    {
        ...
        public void CheckBloonHealth()
        {
            foreach (var b in _bloonSingleton.Bloons.ToList())
            {
                ...
                {
                    _bloonSingleton.Bloons.Add(new BlueBloon { Position = b.Position, Checkpoint = b.Checkpoint, DistanceTravelled = b.DistanceTravelled });
                    _bloonSingleton.Bloons.Remove(b);
                }

                if (b.Color.ToString() == Color.Blue.ToString() && b.Health == 1)
                {
                    _bloonSingleton.Bloons.Add(new RedBloon { Position = b.Position, Checkpoint = b.Checkpoint, DistanceTravelled = b.DistanceTravelled });
                    _bloonSingleton.Bloons.Remove(b);
                }
